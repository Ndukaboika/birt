...
**birt** (Bayesian IRT) fits Item Response Theory models using full Bayesian
estimation via [CmdStan](https://mc-stan.org/users/interfaces/cmdstan). It
gives you posterior distributions, credible intervals, and convergence
diagnostics for every parameter — not just point estimates. Stan models are
...
- **Three models**: Rasch (1PL), 2PL, and 3PL for dichotomous response data
- **Full Bayesian inference**: Credible intervals and posterior distributions
  via Hamiltonian Monte Carlo
- **User-configurable priors**: Weakly informative defaults that work out of
  the box, with class-level and per-item control for users who want
  informative priors
...
- **Real data included**: Algebra test dataset (1,382 students, 12 items)
...
Default priors are weakly informative and work for most tests. Override them
...
- **[Getting Started](https://ndukaboika.github.io/birt/articles/getting-started.html)**
  — Full tutorial covering all three models, priors, diagnostics, and plots
...
- Stan Development Team (2024). *Stan User's Guide*, §1.11: Item-Response
  Theory Models.

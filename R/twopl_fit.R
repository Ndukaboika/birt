...
#' Extends the Rasch model by adding an item discrimination parameter (a)
#' that controls how steeply each item's ICC rises.
#'
#' #' The model is:
...
#'   - a_k = discrimination of question k (NEW vs Rasch)
#'
...
#' fit <- twopl_fit(sim$data, seed = 123)
#' summary(fit)
#' }
#'
#' @export
twopl_fit <- function(data,
  ...
  # --- Compile ---
  stan_file <- system.file("stan", "twopl.stan", package = "birt")
  if (stan_file == "") {
    cli::cli_abort("Stan model file not found. Is {.pkg birt} installed correctly?")
  }

  cli::cli_inform("Compiling 2PL model...")
